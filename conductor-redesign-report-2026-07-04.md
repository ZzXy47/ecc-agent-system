...
**关键变更**:
- 明确使用 `runSubagent` 工具实际调度子 Agent
...
**关键变更**:
- Phase 2（设计）: 明确使用 `runSubagent("planner", ...)` 委托规划
- Phase 3（分发）: 明确使用 `runSubagent("agentName", taskEnvelope)` 分发
- Phase 4（监控）: 明确使用 `manage_todo_list` 追踪进度
- Phase 6（交付）: 明确使用 `runSubagent("code-reviewer", ...)` 和 `runSubagent("security-reviewer", ...)` 执行质量门禁
...
**关键内容**:
- 示例 1: 单任务分发（`runSubagent` 调用格式）
- 示例 2: 并行分发（同一 tool call block 中多个 `runSubagent`）
...
**关键变更**:
- 步骤 1: 明确解析 `runSubagent` 返回结果
- 步骤 3: 新增状态提取步骤（从返回结果中提取 PASS/FAIL/PARTIAL）
- 步骤 7: 明确使用 `runSubagent("code-reviewer", ...)` 运行合并验证
...
**改造前**: 描述性检查清单  
**改造后**: 使用 `runSubagent` 和 `run_in_terminal` 实际执行检查

**可执行检查流程**:
1. 并行执行 `runSubagent("code-reviewer", ...)` 和 `runSubagent("security-reviewer", ...)`
...
**改造前**: 描述性 fallback 链  
**改造后**: 使用 `runSubagent` 实际执行 fallback
...
**改造前**: 描述性检查项  
**改造后**: 使用 `runSubagent("Explore", ...)` 实际执行检查
...
| 缺陷 | 等级 | 改造前 | 改造后 |
|------|------|--------|--------|
| 无 Agent 调度引擎 | P0 | ❌ 描述性规范 | ✅ `runSubagent` 实际调度 |
...
3. **并行执行**: `runSubagent` 同步执行，无法真正异步
4. **上下文隔离**: 子 Agent 无法直接访问父 Agent 上下文

### 8.2 模型行为依赖

1. **调度执行**: 依赖模型正确使用 `runSubagent` 工具
...
2. **规划**: 使用 `runSubagent("planner", ...)` 委托规划
3. **GateGuard 检查**: 使用 `grep_search` 和 `read_file` 收集事实
4. **Safety Guard 检查**: 使用 `grep_search` 检测危险命令
5. **分发任务**: 使用 `runSubagent("agentName", taskEnvelope)` 分发
6. **监控进度**: 使用 `manage_todo_list` 追踪进度
7. **聚合结果**: 解析 `runSubagent` 返回结果
8. **质量门禁**: 使用 `runSubagent("code-reviewer", ...)` 和 `runSubagent("security-reviewer", ...)` 执行检查
9. **交付报告**: 生成结构化交付报告
